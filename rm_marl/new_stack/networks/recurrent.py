...
class CustomLSTMModule(TorchRLModule, ValueFunctionAPI):
    ...
    def setup(self):
        ...
        self._lstm_cell_size = self.model_config.get("lstm_cell_size")
        self._lstm = nn.LSTM(self._num_labels, self._lstm_cell_size, batch_first=True)

        # Assume a simple Box(1D) tensor as input shape.
        in_size = self.observation_space.shape[0] - self._num_labels + self._lstm_cell_size
    ...
    def _compute_embeddings_and_state_outs(self, batch):
        ...
        obs_non_rec = obs[:,:, :-self._num_labels]
        obs_rec = obs[:,:, -self._num_labels:]
        state_in = batch[Columns.STATE_IN]
        h, c = state_in["h"], state_in["c"]

        # Unsqueeze the layer dim (we only have 1 LSTM layer).
        embeddings, (h, c) = self._lstm(obs_rec, (h.unsqueeze(0), c.unsqueeze(0)))
        embeddings = torch.cat((obs_non_rec, embeddings), dim=-1)
        # Push through our FC net.
        embeddings = self._fc_net(embeddings)
        
        # if embeddings.shape[1] != 1 or embeddings.shape[0] != 1:
        #     breakpoint()
