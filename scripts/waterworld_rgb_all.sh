...
noise_levels=(0.9990105628967285 0.9977762699127197 0.997037410736084)
...
directory="waterworld_rgb_unrestricted_all"
    ...
    for noise_level in "${noise_levels[@]}"; do
      ...
      python submit_rcs_script.py ${nodes} ${ncpus} ${ram} ${directory} ${name} \
        ray_tests/hydra_RM_learning_PPO.py env/water-world@env=red_green_blue run.name=${name} \
          ...
          run.use_perfect_rm=${use_rm} run.num_agents=2 run.should_tune=True \
              run.tune_config.num_samples=1 \
          run.num_env_runners=30 run.stop_iters=10000 \
          ...
          +experiment=vanilla_ww_all_symmetric_error x=${noise_level} 
    done
  done
done
