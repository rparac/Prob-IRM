...
directory="rm_learning_deliver_coffee"
for num_agent in "${num_agents[@]}"; do
  for noise_level in "${noise_levels[@]}"; do
    ...
    python submit_rcs_script.py ${nodes} ${ncpus} ${ram} ${directory} ${name} \
      ray_tests/hydra_RM_learning_PPO.py run.name=${name} \
        run.use_perfect_rm=False run.num_agents=${num_agent} run.should_tune=True \
	run.num_env_runners=20 run.tune_config.num_samples=1 \
