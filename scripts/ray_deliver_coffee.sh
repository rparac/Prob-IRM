...
for seed in "${seeds[@]}"; do
  for noise_level in "${noise_levels[@]}"; do
    ...
    cmd="python submit_rcs_old_script.py ${ncpus} ${ram} ${directory} ${name} ray_tests/RM_learning_PPO.py --enable-new-api-stack --stop-iters=500"
    if [ "$wandb_name" = "single_agent_rm" ]; then
      ...
      cmd="$cmd --use-perfect-rm --num-samples 5000" # --wandb-run-name=single_agent_tune"
    else
      cmd=cmd
    fi
