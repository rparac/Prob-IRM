...
def create_config(
    ...
    config = (
        ...
        .framework("torch")
        .training(
            lr=0.00042400891937438285, # tune.loguniform(1e-5, 1e-3),  # Learning rate on a log scale
            gamma=0.99,
        )
        .training(
            ...
            kl_coeff=0.7075314346062174, # tune.uniform(0.3, 1),
            num_sgd_iter=3,  #tune.randint(3, 30),  # Number of epochs to execute per training batch
            lambda_=0.9654838493721581, # tune.uniform(0.9, 1),
            vf_loss_coeff=0.89569119116459, # tune.uniform(0.5, 1),
            entropy_coeff=0.0979929338527027, # tune.uniform(0.0, 0.1),
            grad_clip=100.0,
    ...
    config.rl_module(
        ...
        model_config_dict={
            ...
            "fcnet_hiddens": [32], # tune.choice(hidden_architectures),
            "fcnet_activation": "elu", # tune.choice(["relu", "tanh", "elu"]),
        }
    )
